...
def render_issue(report_name: str, result: dict, modules: list[str]) -> str:
    ...
    if present:
        suffix_lines.append("")
        suffix_lines.append("<details><summary>Left as genuine findings (vulnerable package IS linked) — "
                            f"{len(present)} rows</summary>")
        suffix_lines.append("")
        suffix_lines.append("See `vex-candidates.json` in the workflow artifacts for the full row-level detail.")
        suffix_lines.append("</details>")
    ...
    if undet:
        ...
        suffix_lines.append("")
        suffix_lines.append("<details><summary>Undetermined (needs manual review) — "
                            f"{len(undet)} rows</summary>")
        suffix_lines.append("")
        for r, n in sorted(reasons.items()):
            suffix_lines.append(f"- {r}: {n}")
        suffix_lines.append("")
        suffix_lines.append("</details>")

    suffix_lines.append("")
    suffix_lines.append("<sub>Generated by `.github/scripts/vex_candidates.py`. Validate before transferring "
    ...
    candidate_rows = render_textvex(cands).splitlines()
    run_url = github_run_url()
    tail_lines = ["```", ""]
    if run_url:
        tail_lines.append(f"Full analysis artifacts for this run: {run_url}")
    else:
        tail_lines.append("Full analysis is available in the uploaded workflow artifacts.")

    max_rows = len(candidate_rows)
    while max_rows > 0:
        body_lines = lines + candidate_rows[:max_rows] + tail_lines
        if max_rows < len(candidate_rows):
            omitted = len(candidate_rows) - max_rows
            body_lines.extend([
                "",
                f"Truncated for the GitHub issue size limit; {omitted} additional rows are in the workflow artifacts.",
            ])
        body = "\n".join(body_lines + suffix_lines)
        if len(body) <= GITHUB_ISSUE_BODY_LIMIT:
            break
        max_rows -= 1

    lines.extend(candidate_rows[:max_rows])
    lines.append("```")
    if max_rows < len(candidate_rows):
        omitted = len(candidate_rows) - max_rows
        lines.append("")
        lines.append(f"Truncated for the GitHub issue size limit; {omitted} additional rows are in the workflow artifacts.")
    lines.extend(tail_lines[1:])
    return "\n".join(lines + suffix_lines)
