...
EOF
    ...
    if [[ "$use_prime_ingress" == "true" ]]; then
        ...
        # images-ingress-nginx.txt (master) or folded into images-core.txt
        # (release branches). Rewrite the hardened tag to the prime tag wherever
        # it appears across the generated lists. The hardened tag only applies to
        # the ingress-nginx images, so this is safe regardless of layout.
        prime_rewrote="false"
